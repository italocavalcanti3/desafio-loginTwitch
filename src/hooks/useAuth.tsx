import { AuthRequestPromptOptions, makeRedirectUri, revokeAsync, startAsync } from 'expo-auth-session';
  ...
  async function signIn() {
    try {
      ...
      if (response.type === 'success' && response.params.error !== 'access_denied') {
        ...
        const userResponseFormatted = userResponse.data.data[0];
        
        setUser({
          id: userResponseFormatted.id,
          display_name: userResponseFormatted.display_name,
          email: userResponseFormatted.email,
          profile_image_url: userResponseFormatted.profile_image_url
        });
